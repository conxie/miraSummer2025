...
penguins %>%
  ggplot(aes(x = body_mass_g, fill = species)) +
  geom_histogram(color = "black", bins = 25) + 
  geom_vline(aes(xintercept = 3750), color = "blue", linetype = "dashed") + 
  geom_vline(aes(xintercept = 4350), color = "magenta", linetype = "dashed")
#blue = mode
#magenta = mean
```
...
penguins %>%
  ggplot(aes(y = flipper_length_mm, 
             x = bill_length_mm)) +
  geom_violin(aes(color = species), alpha = 0) +
...
penguins %>%
  ggplot(aes(y = flipper_length_mm, x = species)) + 
  geom_hex(color = "blue", size = 0.3, alpha = 0.8) + 
  geom_jitter()
```
...
penguins %>%
  ...
  ggridges::geom_density_ridges(scale = 0.7, 
                                color = "pink", 
                                fill = "white")
```
...
```{r}
penguins %>%
    ggplot(aes(y = flipper_length_mm, x = factor(species), 
               group = factor(sex), color = factor(species))) +
