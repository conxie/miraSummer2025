...
if(confidence >= 6) {
  print("You are in good shape!")
} else if (confidence< 6) {print("Better practice some more!")}
```
...
```{r}
if(confidence >= 8) {print("You are in great shape!")} else
  if(4 <= confidence & confidence <= 7) {print("Good, keep at it!")} else
...
```{r}
case_when(confidence >= 6 ~ "You are in good shape!",
          confidence < 6 ~ "Better practice some more!")
```

8. Rewrite your code for 4 using `case_when()`.

```{r}
case_when(confidence >= 8 ~ "You are in great shape!",
...
```{r}
hobbies <- c("Coffee", "sourdough", "Bread", "TK", "AC", "QG", "Puzzles", "Anxiety")
...
```{r}
participated <- c(TRUE, FALSE, TRUE, FALSE, FALSE, FALSE, FALSE, TRUE)
```
  
  B. Combine `hobbies`, `ratings`, and `participated` to create a dataframe called `pandemic_hobbies`. It should have three columns, titled "hobby", "rating", and "participated".
  
```{r}
pandemic_hobbies <- data.frame(Hobbies = hobbies, Rating = rating, Participated = participated)

print(pandemic_hobbies)
```
